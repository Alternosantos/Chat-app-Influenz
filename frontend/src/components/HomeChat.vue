    ...
    <div class="chat-wrapper">
      ...
      <div class="messages-container">
        ...
        <div class="messages" ref="messages">
          <div v-for="(msg, index) in activeMessages" 
               :key="index" 
...
export default {
  ...
  computed: {
    ...
    activeMessages() {
      ...
      return this.messages.filter(msg =>
        (msg.sender === this.sender && msg.recipient === this.selectedUser.name) ||
        (msg.recipient === this.sender && msg.sender === this.selectedUser.name)
  ...
  methods: {
    connectWebSocket() {
      ...
      this.ws.onmessage = (event) => {
        ...
        if (message.type === 'activeUsers') {
          ...
          if ((message.sender === this.selectedUser?.name && message.recipient === this.sender) ||
              (message.recipient === this.selectedUser?.name && message.sender === this.sender)) {
    ...
    updateLastMessage(message) {
      const user = this.users.find(u => u.name === message.sender || u.name === message.recipient)
      if (user) {
